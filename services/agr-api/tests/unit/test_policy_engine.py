...
when { resource has environment && resource.environment == "production" };
    ''')
    result = evaluate_policies([policy], "agent-1", "db.drop", "mydb",
                                {"environment": "production"})
    assert result.decision == "DENY"
...
when { resource has environment && resource.environment == "production" };
    ''')
    result = evaluate_policies([policy], "agent-1", "db.truncate", "mydb",
                                {"environment": "production"})
    assert result.decision == "DENY"
...
def test_forbid_overrides_permit() -> None:
    permit = _make_policy("p1", '''
permit(principal, action == Action::"deploy", resource)
when { resource has environment && resource.environment == "production" };
    ''')
    forbid = _make_policy("p2", '''
forbid(principal, action == Action::"deploy", resource)
when { resource has environment && resource.environment == "production" };
    ''')
    result = evaluate_policies([permit, forbid], "agent-1", "deploy", "server",
                                {"environment": "production"})
    assert result.decision == "DENY"
...
when { resource has environment && resource.environment == "production" };
    ''')
    result = evaluate_policies([p1, p2], "agent-1", "deploy", "server",
                                {"environment": "staging"})
    assert result.decision == "ALLOW"
    assert result.policy_id == "p1"
...
when { resource has environment && resource.environment == "production" };
    ''')
    result = evaluate_policies([p1, p2], "agent-1", "fs.write", "file",
                                {"path": "/src/main.py"})
    assert result.decision == "ALLOW"
...
def test_result_dataclass_properties() -> None:
    result = EvaluationResult(
        decision="ALLOW", reason="test", policy_id="p1",
        requires_approval=False, latency_ms=0.5
    )
    assert result.decision == "ALLOW"
    assert result.policy_id == "p1"
